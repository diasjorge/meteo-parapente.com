...
class JavaScriptPacker {
	...
	private function _encodeSpecialChars($script) {
		...
		// build the word list
		$o_keywords = $this->_analyze($script, $regexp, '_encodePrivate');
		// quick ref
		$l_encoded = $o_keywords['encoded'];
	...
	private function _encodeKeywords($script) {
		...
		// create the parser
		$parser = new ParseMaster();
		$l_encode = $this->_getEncoder($this->_encoding);
		// for high-ascii, don't encode single character low-ascii
		$regexp = ($this->_encoding > 62) ? '/\\w\\w+/' : '/\\w+/';
		// build the word list
		$o_keywords = $this->_analyze($script, $regexp, $l_encode);
		$l_encoded = $o_keywords['encoded'];
		...
		if (empty($script)) return $script;
			...
			//$res = $this->_bootStrap($res, $o_keywords);
			//return $res;
			return $this->_bootStrap($parser->exec($script), $o_keywords);
		}
	}
	
	private function _analyze($script, $regexp, $l_encode) {
		// analyse
		...
		if (!empty($all)) {
			...
			do {
				...
				if (!isset($this->_count[$word])) {
					...
					//if (is_string($l_encode) && method_exists($this, $l_encode))
					$values[$j] = call_user_func(array(&$this, $l_encode), $j);
	...
	private function _bootStrap($n_packed, $o_keywords) {
		$ENCODE = $this->_safeRegExp('$l_encode\\($count\\)');

		// $n_packed: the packed script
		$n_packed = "'" . $this->_escape($n_packed) . "'";

		// $i_ascii: base for encoding
		$i_ascii = min(count($o_keywords['sorted']), $this->_encoding);
		if ($i_ascii == 0) $i_ascii = 1;

		// $count: number of words contained in the script
		$count = count($o_keywords['sorted']);

		// $o_keywords: list of words contained in the script
		foreach ($o_keywords['protected'] as $i=>$value) {
			$o_keywords['sorted'][$i] = '';
		}
		// convert from a string to an array
		ksort($o_keywords['sorted']);
		$o_keywords = "'" . implode('|',$o_keywords['sorted']) . "'.split('|')";

		$l_encode = ($this->_encoding > 62) ? '_encode95' : $this->_getEncoder($i_ascii);
		$l_encode = $this->_getJSFunction($l_encode);
		$l_encode = preg_replace('/_encoding/','$i_ascii', $l_encode);
		$l_encode = preg_replace('/arguments\\.callee/','$l_encode', $l_encode);
		$inline = '\\$count' . ($i_ascii > 10 ? '.toString(\\$i_ascii)' : '');

		// $a_decode: code snippet to speed up decoding
		if ($this->_fastDecode) {
			// create the decoder
			$a_decode = $this->_getJSFunction('_decodeBody');
			if ($this->_encoding > 62)
				$a_decode = preg_replace('/\\\\w/', '[\\xa1-\\xff]', $a_decode);
			// perform the encoding inline for lower ascii values
			elseif ($i_ascii < 36)
				$a_decode = preg_replace($ENCODE, $inline, $a_decode);
			// special case: when $count==0 there are no keywords. I want to keep
			//  the basic shape of the unpacking funcion so i'll frig the code...
			if ($count == 0)
				$a_decode = preg_replace($this->_safeRegExp('($count)\\s*=\\s*1'), '$1=0', $a_decode, 1);
		}
		...
		if ($i_ascii > 36 || $this->_encoding > 62 || $this->_fastDecode) {
			// insert the encode function
			$this->buffer = $l_encode;
		...
		// arguments
		$params = array($n_packed, $i_ascii, $count, $o_keywords);
		if ($this->_fastDecode) {
			$params[] = 0;
			$params[] = '{}';
		}
	...
	private function _insertFastEncode($match) {
		return '{$l_encode=' . $this->buffer . ';';
	}
	
	// mmm.. ..which one do i need ??
	private function _getEncoder($i_ascii) {
		return $i_ascii > 10 ? $i_ascii > 36 ? $i_ascii > 62 ?
...
'function($n_packed, $i_ascii, $count, $o_keywords, $l_encode, $a_decode, s) {
    while ($count--) {
        if ($o_keywords[$count]) {
            $n_packed = $n_packed.replace(new RegExp(\'\\\\b\' + $l_encode($count) + \'\\\\b\', \'g\'), $o_keywords[$count]);
        }
    }
    return $n_packed;
}';
/*
'function($n_packed, $i_ascii, $count, $o_keywords, $l_encode, $a_decode) {
    while ($count--)
        if ($o_keywords[$count])
            $n_packed = $n_packed.replace(new RegExp(\'\\\\b\' + $l_encode($count) + \'\\\\b\', \'g\'), $o_keywords[$count]);
    return $n_packed;
...
'    if (!\'\'.replace(/^/, String)) {
        ...
        while ($count--) {
            $a_decode[$l_encode($count)] = $o_keywords[$count] || $l_encode($count);
        }
        // global replacement function
        $o_keywords = [function ($l_encoded) {return $a_decode[$l_encoded]}];
        // generic match
        $l_encode = function () {return \'\\\\w+\'};
...
'	if (!\'\'.replace(/^/, String)) {
        // decode all the values we need
        while ($count--) $a_decode[$l_encode($count)] = $o_keywords[$count] || $l_encode($count);
        // global replacement function
        $o_keywords = [function ($l_encoded) {return $a_decode[$l_encoded]}];
        // generic match
        $l_encode = function () {return\'\\\\w+\'};
